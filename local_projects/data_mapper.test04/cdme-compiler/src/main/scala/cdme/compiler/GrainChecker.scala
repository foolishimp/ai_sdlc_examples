...
object GrainChecker {
  ...
  def checkTransition(
    ...
    if (order.areSameLevel(sourceGrain, targetGrain))
      // Same grain level: always safe
      Right(())
    else if (!order.areComparable(sourceGrain, targetGrain))
      Left(ValidationError.GrainViolation(
        s"Grains ${sourceGrain.show} and ${targetGrain.show} are not comparable"
      ))
    else if (order.isFinerThan(sourceGrain, targetGrain))
  ...
  def validatePath(
    ...
    transitions.flatMap { case (src, tgt, card) =>
