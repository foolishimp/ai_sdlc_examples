...
import cdme.model.category._
import cdme.model.adjoint._
import cdme.model.grain._
import cdme.model.types.{PrimitiveType, SemanticType, SemanticTag, SubtypeRegistry}
import cdme.model.context._
import cdme.model.access._
import cdme.model.error._
...
class AcceptanceCriteriaSpec extends AnyFunSuite with Matchers with ScalaCheckPropertyChecks {
  ...
  test("AC-CDME-008: |input| = |processed| + |filtered| + |errored| for every run") {
    forAll(Gen.choose(1, 999)) { (input: Int) =>
  ...
  test("AC-CDME-009: every target value traces to source epoch, entity, and morphism path") {
    ...
    val error = ErrorObject(
      ...
      morphismPath = List(
        MorphismId("tradeToCP"),
        MorphismId("cpToRegion")
      ),
