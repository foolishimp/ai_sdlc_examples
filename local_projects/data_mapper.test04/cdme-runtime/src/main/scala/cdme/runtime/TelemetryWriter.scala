...
object TelemetryEntry {
  ...
  implicit val telemetryVectorMonoid: Monoid[Vector[TelemetryEntry]] = new Monoid[Vector[TelemetryEntry]] {
...
final case class WriterEffect[W, A](run: (A, W)) {
   ...
   */
  def flatMap[B](f: A => WriterEffect[W, B])(implicit m: Monoid[W]): WriterEffect[W, B] = {
    val next = f(run._1)
...
object WriterEffect {
   ...
   */
  def pure[W, A](a: A)(implicit m: Monoid[W]): WriterEffect[W, A] =
